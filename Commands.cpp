...
>>>>>>> 466da0091c1d7d611b1e75d56348067d21788860
		return (0);
	std::string temp_args = this->args; 
	while(temp_args[found] == ' '){
		found++;
...
int	Server::validArgsPriv(){
	std::string	message;
	size_t		count = 0;
	size_t		ind;
	std::vector<std::string>	vec;
	if (this->args == "\0"){//FIXED
		std::cout << "no args for privmsg!" << std::endl;
		return (0);
	}
	size_t	index = this->args.find_first_of(" \t\r','");
	...
	if (this->args[index] == ','){
		...
		for (size_t i = 0; i <= count; i++){
			std::cout << "index:" << (ind - start) << std::endl;
			vec.push_back(this->args.substr(start, ind - start));
			start = ind + 1;
			ind = this->args.find_first_of("','", start);
			if (ind == std::string::npos)
				ind = this->args.find_first_of(" \t\r");	
		}
	...
	message = (this->args.substr(i, this->args.size()));
	std::string	target = this->args.substr(0, index);
...
void Server::handleCommands1(void){
	if (this->command == "join"){
		...
		int n = validArgsPriv();
		std::cout << "return dial validArgsPriv:" << n << "+++" << std::endl;
		if (validArgsPriv())
			std::cout << "message sent hourray!" << std::endl;
		else{
			// std::cout << "validArgsPriv siftat 0" << std::endl;
			if (send(this->connectionID, "Invalid args for privmsg\n", 25, 0) == -1)
				throw(std::runtime_error("Failed to send to client!"));
		}
